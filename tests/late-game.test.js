...
assert.strictEqual(Sim.canAdvance(chaosKing), true, '混沌体没有圣位门槛，不能卡死在王者');
assert.ok(Sim.noRealmBottleneck(chaosKing), '混沌体境界无瓶颈');
const daoEmbKing = Sim.createGame(0, []);
Sim.setPhysique(daoEmbKing, DATA.physiqueById('innate_sacred_dao'));
daoEmbKing.lvl = 70;
daoEmbKing.daoyun = 0;
assert.strictEqual(Sim.canAdvance(daoEmbKing), true, '先天圣体道胎也不能卡在圣位');
