...
(function () {
  ...
  var afterSit = Sim.createGame(0, []);
  Sim.setPhysique(afterSit, D.physiqueById('mortal'));
  afterSit.innate = 1;
  afterSit.aptitude = 1;
  afterSit.lvl = 48;
  afterSit.age = 400;
  afterSit.eventDrawsBySpan = { pre: 0 };
  afterSit.recentEvents = [{ id: 'th_stuck_xian', tag: 'insight' }];
  var afterSitRnd = Math.random;
  Math.random = function () { return 0.9; };
  Sim.rollEvent(afterSit, []);
  ...
  var nengG = Sim.createGame(0, []);
  Sim.setPhysique(nengG, D.physiqueById('mortal'));
  nengG.innate = 1;
  nengG.aptitude = 1;
  nengG.daoGift = 5;
  nengG.lvl = 56;
  nengG.daoyun = 80;
  nengG.daoyunCap = 400;
  var nengBefore = nengG.lvl;
  neng.ok(nengG, Sim.U);
  assert.ok(nengG.lvl >= nengBefore + 2, '大能调息在内层应连破两层，实际 ' + nengG.lvl);
  var nengEdge = Sim.createGame(0, []);
  Sim.setPhysique(nengEdge, D.physiqueById('mortal'));
  nengEdge.innate = 1;
  nengEdge.aptitude = 1;
  nengEdge.daoGift = 5;
  nengEdge.lvl = 59;
  nengEdge.daoyun = 80;
  nengEdge.daoyunCap = 400;
  neng.ok(nengEdge, Sim.U);
  assert.strictEqual(nengEdge.lvl, 60, '大能尽头连破也不能坐进斩道，实际 ' + nengEdge.lvl);
  var kingInner = Sim.createGame(0, []);
  Sim.setPhysique(kingInner, D.physiqueById('mortal'));
  kingInner.innate = 1;
  kingInner.aptitude = 1;
  kingInner.daoGift = 5;
  kingInner.lvl = 65;
  kingInner.cutDaoPassed = true;
  kingInner.daoyun = 120;
  kingInner.daoyunCap = 500;
  var kingBefore = kingInner.lvl;
  neng.ok(kingInner, Sim.U);
  assert.ok(kingInner.lvl >= kingBefore + 2, '王者内层调息也应连破两层，实际 ' + kingInner.lvl);
  var kingEdge = Sim.createGame(0, []);
  Sim.setPhysique(kingEdge, D.physiqueById('mortal'));
  kingEdge.innate = 1;
  kingEdge.aptitude = 1;
  kingEdge.daoGift = 5;
  kingEdge.lvl = 69;
  kingEdge.cutDaoPassed = true;
  kingEdge.daoyun = 120;
  kingEdge.daoyunCap = 500;
  neng.ok(kingEdge, Sim.U);
  assert.ok(kingEdge.lvl >= 70 && kingEdge.lvl < 71, '王者尽头连破也不能坐进入圣，实际 ' + kingEdge.lvl);
